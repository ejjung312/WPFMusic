...
public class SerializeHelper
{
    public static T ReadDataFromXmlFile<T>(string fileName, bool useDataContractSerialize = false) where T : class
    {
		try
		{
            // using: 객체의 자원을 자동으로 해제하여 메모리 누수를 방지.
		...
		{
            Logger.Log.Error(ex); // 이거 어떻게 해결해?
            return null;
        }
    }

    private static T DataContractSerializeDeserialize<T>(string xmlData) where T : class
    {
        try
        {
    ...
    private static T XmlSerializerDeserialize<T>(string xmlData) where T : class
    {
        try
        {
